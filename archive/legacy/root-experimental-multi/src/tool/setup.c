    ...
    )
{
    ...
    // Reserve additional space for adding our service name with overflow protection
    if (cbRequired > (SIZE_MAX - sizeof(_wszFilterName))) {
        fprintf(stderr, "Buffer size too large, last error is %lu.\n", GetLastError());
        goto Cleanup;
    }
    ...
    for (pwszCurrent = pwszUpperFilters; *pwszCurrent; pwszCurrent += wcslen(pwszCurrent) + 1)
    {
        if (_wcsicmp(pwszCurrent, _wszFilterName) == 0)
        {
            ...
            if ((pwszCurrent + (sizeof(_wszFilterName) / sizeof(WCHAR)) + 1) <= (pwszUpperFilters + cbAllocate / sizeof(WCHAR))) {
                CopyMemory(pwszCurrent, _wszFilterName, sizeof(_wszFilterName));
                pwszCurrent += sizeof(_wszFilterName) / sizeof(WCHAR);
                *pwszCurrent = 0;
            } else {
                fprintf(stderr, "Buffer overflow prevented in UpperFilters update.\n");
                goto Cleanup;
            }
...
{
    int rc;
    rc = CheckInstallation();
    if (rc != 0) return rc;
    return EnumMonitorablePorts(AttachPortCallback, NULL);
}

int HandleDetachAllParameter(void)
{
    return EnumMonitorablePorts(DetachPortCallback, NULL);
}
