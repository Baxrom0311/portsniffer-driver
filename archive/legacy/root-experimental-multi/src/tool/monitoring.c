...
{
    USHORT expected, calc;
    
    if (len < 3) return FALSE;
    expected = (USHORT)(data[len - 2] | (data[len - 1] << 8));
    calc = _Crc16Texno(data, len - 2);
    ...
    )
{
    ...
    WCHAR json[16384];  // Increased buffer size to prevent overflow
    char utf8Json[16384];
    int utf8Len;
    BOOL ok;
    TEXNO_DECODE d;
    ...
    if (!h) { 
        fprintf(stderr, "winhttp.dll not found.\n"); 
        return FALSE; 
    }
    ...
    _DecodeTexno(pEntry->Data, pEntry->DataLength, &d);
    ...
    {
        // Build basic JSON without Texno UZ fields
        StringCchPrintfW(json, _countof(json),
    ...
    // Convert JSON to UTF-8 for HTTP request
    utf8Len = WideCharToMultiByte(CP_UTF8, 0, json, -1, utf8Json, sizeof(utf8Json), NULL, NULL);
    if (utf8Len <= 0)
    {
        fprintf(stderr, "WideCharToMultiByte failed, last error is %lu.\n", GetLastError());
        pClose(hConn); pClose(hSession); FreeLibrary(h); return FALSE;
    }

    // Send HTTP request
    ok = pSend(hReq, L"Content-Type: application/json\r\n", -1, utf8Json, utf8Len - 1, utf8Len - 1, 0);
    if (!ok) { fprintf(stderr, "WinHttpSendRequest failed, last error is %lu.\n", GetLastError()); pClose(hReq); pClose(hConn); pClose(hSession); FreeLibrary(h); return FALSE; }

    ok = pRecv(hReq, NULL);
    if (!ok) { fprintf(stderr, "WinHttpReceiveResponse failed, last error is %lu.\n", GetLastError()); pClose(hReq); pClose(hConn); pClose(hSession); FreeLibrary(h); return FALSE; }
    ...
    )
{
    ...
    while (!_bTerminationRequested)
    {
        ...
        for (iPortIdx = 0; iPortIdx < nPortCount; iPortIdx++)
        {
            ...
            {
                canCopy = pPopResponse->DataLength;
