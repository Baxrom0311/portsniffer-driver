...
_PrintUsage()
{
    ...
    printf("\n");
    printf("Monitoring:\n");
    printf("    /monitor PORT TYPES [/forward URL]\n");
    printf("                            Monitor the given port. Optionally forward entries to URL.\n");
    ...
    )
{
    ...
    else if (argc >= 2 && wcscmp(argv[1], L"/monitor") == 0)
    {
        if (argc == 4)
        {
            return HandleMonitorParameter(argv[2], argv[3], NULL);
        }
        else if (argc == 6 && _wcsicmp(argv[4], L"/forward") == 0)
        {
            return HandleMonitorParameter(argv[2], argv[3], argv[5]);
        }
        else
        {
            return _PrintUsage();
