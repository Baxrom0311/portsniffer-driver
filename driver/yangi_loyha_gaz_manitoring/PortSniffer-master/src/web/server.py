...
def _check_packet(data: bytes) -> bool:
  if len(data) < 3:
    return False
  expected = int.from_bytes(data[-2:], byteorder="little", signed=False)
...
def data():
  ...
  if q_combine:
    ...
    for e in src_chrono:
      ...
      # Merge into current
      cur["data_hex"] = (cur.get("data_hex", "") + e.get("data_hex", ""))
      cur["data_text"] = (cur.get("data_text", "") + e.get("data_text", ""))
      try:
